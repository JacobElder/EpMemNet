...
```{r}
rawDf <- data.table::fread("/Volumes/Research Project/Episodic Memory Network/Study 3/input/epmnRaw3.csv")
...
```{r}
rawDfC <- subset(rawDf, NType=="C")

  uIds <- unique(rawDfC$subID)
  # column names
  idSubsC <- rawDfC %>%
  ...
  for(n in 1:nrow(rawDfC)){
    ...
    for(c in 1:numMems){
      ...
      selectWord <-strsplit(as.character(idSubsC[n,c][[1]]),",") 
      ...
      if(subMatW[as.numeric(unlist(p)),c]==0){
        ...
        subMatW[as.numeric(unlist(p)),c]  <- as.numeric ( rawDfC[n,paste0("M",c,"_Sim_",as.numeric(unlist(p) ))] ) # Weighted matrix
        
      }else if(subMatW[as.numeric(unlist(p)),c]>0){
        
        subMatW[as.numeric(unlist(p)),c]  <- mean(c(subMatW[as.numeric(unlist(p)),c], as.numeric ( rawDfC[n,paste0("M",c,"_Sim_",as.numeric(unlist(p) ))] )  )) # Weighted matrix
...
# Associativist Network Creation

```{r}
rawDfA <- subset(rawDf, NType=="A")

  uIds <- unique(rawDfA$subID)
  # column names
  idSubsA <- rawDfA %>%
    select(contains("_Relations.A"))

  # Subjects to remove based on network
  removeSubsA <- c()
  for(n in 1:nrow(rawDfA)){
    i <- rawDfA$subID[n]
    ...
    for(c in 1:numMems){
      ...
      j <- idSubsA[n,c]
      # split words by comma
      selectWord <-strsplit(as.character(idSubsA[n,c][[1]]),",") 
      if(length(selectWord[[1]])==0){
        next
      }
      ...
      # populate 1s for select identities in row and column for undirected
      subMat[as.numeric(unlist(selectWord)),c] <- 1 # Unweighted matrix
      # populate both directions for undirected network
      ...
      if(subMatW[as.numeric(unlist(p)),c]==0){
        ...
        subMatW[as.numeric(unlist(p)),c]  <- as.numeric ( rawDfA[n,paste0("M",c,"_Sim.A_",as.numeric(unlist(p) ))] ) # Weighted matrix
        subMatW[c,as.numeric(unlist(p))]  <- as.numeric ( rawDfA[n,paste0("M",c,"_Sim.A_",as.numeric(unlist(p) ))] ) # Weighted matrix
        
      }else if(subMatW[as.numeric(unlist(p)),c]>0){
        
        subMatW[as.numeric(unlist(p)),c]  <- mean(c(subMatW[as.numeric(unlist(p)),c], as.numeric ( rawDfA[n,paste0("M",c,"_Sim.A_",as.numeric(unlist(p) ))] )  )) # Weighted matrix
        
        subMatW[c,as.numeric(unlist(p))]  <- mean(c(subMatW[c,as.numeric(unlist(p))], as.numeric ( rawDfA[n,paste0("M",c,"_Sim.A_",as.numeric(unlist(p) ))] )  )) # Weighted matrix
...
```{r}
removeSubs <- c(removeSubsA, removeSubsC)
...
# Causal Dataframe Organization

```{r}
causalLong <- matrix(nrow=0,ncol=65)

causalShort <- matrix(nrow=0,ncol=53)

uIds <- unique(rawDfC$subID)

colnames(rawDfC)[which(colnames(rawDfC)=="M1_IM"):(which(colnames(rawDfC)=="M1_IM")+22)]
varnames <- c("IM", "Pos", "Neg", "Clear", "Easy", "Rep", "Fund", "Chan", "Know", "Taught", "Expect", "Breadth", "Initiate", "Locus", "Control", "Stability", "SO", "Simulate", "T.Y", "T.M", "D.D")
  ...
  for(j in 1:length(uIds)){
    ...
    for(v in variables){
      aVar <- as.numeric(rawDfC[rawDfC$subID==subID, paste0("M",1:numMems,"_",v)])
      assign(v,aVar)
    }
    ...
    causalLong <- rbind(causalLong, subLong)
    causalShort <- rbind(causalShort, subShort)
  }

causalLong <- as.data.frame(causalLong)
...
colnames(causalLong)[1:length(begNames)] <- begNames

causalLong[4:ncol(causalLong)] <- apply(causalLong[4:ncol(causalLong)], 2, as.numeric)
...
colnames(causalShort)[1:length(begNames)] <- begNames
causalShort <- as.data.frame(causalShort)
...
```{r}
assocLong <- matrix(nrow=0,ncol=57)

assocShort <- matrix(nrow=0,ncol=53)

uIds <- unique(rawDfA$subID)

colnames(rawDfA)[which(colnames(rawDfA)=="M1_IM"):(which(colnames(rawDfA)=="M1_IM")+22)]
varnames <- c("IM", "Pos", "Neg", "Clear", "Easy", "Rep", "Fund", "Chan", "Know", "Taught", "Expect", "Breadth", "Initiate", "Locus", "Control", "Stability", "SO", "Simulate", "T.Y", "T.M", "D.D")
  ...
  for(j in 1:length(uIds)){
    ...
    variables <- c("IM", "Val_1", "Val_2", "Clear", "Easy", "Rep", "Fund", "Chan", "Know", "Taught", "Expect", "Breadth", "Initiate", "Locus", "Control", "Stability", "SO", "Simulate", "Length_12", "Length_13", "Length_14")
    for(v in variables){
      aVar <- as.numeric(rawDfA[rawDfA$subID==subID, paste0("M",1:numMems,"_",v)])
      assign(v,aVar)
    }
    ...
    assocLong <- rbind(assocLong, subLong)
    assocShort <- rbind(assocShort, subShort)
  }

assocLong <- as.data.frame(assocLong)
begNames <- c("subID","memory","memCode","degree", "strength","eigen","eigenW","hub","hubW","page","pageW","pageOut","pageOutW","auth","authW")
colnames(assocLong)[1:length(begNames)] <- begNames

assocLong[4:ncol(assocLong)] <- apply(assocLong[4:ncol(assocLong)], 2, as.numeric)
...
begNames <- c("subID","edgeTot","dense","aveDist","idTrans","globEff","numID",
              ...
              "centrEigW","sdDeg","sdBet","sdClo","sdEig","sdDegW","sdBetW","sdCloW","sdEigW",
              paste0(variables,"_Homoph"))
colnames(assocShort)[1:length(begNames)] <- begNames
assocShort <- as.data.frame(assocShort)
```
...
for(i in 1:nrow(assocLong)){
  assocLong$length[i] <- convertLength(assocLong$Length_12[i], assocLong$Length_13[i], assocLong$Length_14[i])
  
}
assocLong$length[assocLong$length > (365*65)] <- NA # Remove outliers
for(i in 1:nrow(causalLong)){
  causalLong$length[i] <- convertLength(causalLong$Length_12[i], causalLong$Length_13[i], causalLong$Length_14[i])
  
}
causalLong$length[causalLong$length > (365*65)] <- NA # Remove outliers
```
...
for(i in 1:2){
  ...
  if(i == 1){
    fullLong <- causalLong
    fullShort <- causalShort
  }else if(i == 2){
    fullLong <- assocLong
    fullShort <- assocShort
  }
...
fullShort <- fullLong %>%
  ...
  if(i == 1){
    assign("causalLong",fullLong)
    assign("causalShort",fullShort)
  }else if(i == 2){
    assign("assocLong",fullLong)
    assign("assocShort",fullShort)
  }
...
```{r}
arrow::write_parquet(causalLong, here("Study 3/Data/longEpMNetC3.parquet"))
arrow::write_parquet(assocLong, here("Study 3/Data/longEpMNetA3.parquet"))
arrow::write_parquet(causalShort,here("Study 3/Data/shortEpMNetC3.parquet"))
arrow::write_parquet(assocShort,here("Study 3/Data/shortEpMNetA3.parquet"))
arrow::write_parquet(fullSimDf, here("Study 3/Data/simEpMNet3.parquet"))
arrow::write_parquet(rankDf, here("Study 3/Data/rankings3.parquet"))
dir.create("/Volumes/Research Project/Episodic Memory Network/Study 3/workspace/")
save.image("/Volumes/Research Project/Episodic Memory Network/Study 3/workspace/idmWorkspace.RData")
```
