...
```{r}
fullLong <- matrix(nrow=0,ncol=62)

fullShort <- matrix(nrow=0,ncol=34)
...
"Breadth", "SO_1", "SO_2", "Length_12", "Length_13", "Length_14")
    ...
    subLong <- 
      cbind(subID,
          ...
          PANAS_9,
          PANAS_10,
          rowMeans(cbind(PANAS_1,PANAS_2,PANAS_3,PANAS_4,PANAS_5),na.rm=T),
          rowMeans(cbind(PANAS_6,PANAS_7,PANAS_8,PANAS_9,PANAS_10),na.rm=T),
          Fund,
          Clear,
          Rep,
          Chan,
...
"Breadth", "SO_1", "SO_2")
    ...
    subShort <- c(subID,idEdgT,density,aveDist,idTrans,globEff, numMems, meanStre, sumStre, idSimGlob, homophs)
...
begNames <- c("subID","memory","memCode","degree", "outdegree", "indegree", "strength")
colnames(fullLong)[1:length(begNames)] <- begNames

fullLong[4:ncol(fullLong)] <- apply(fullLong[4:ncol(fullLong)], 2, as.numeric)


begNames <- c("subID","edgeTot","dense","aveDist","idTrans","globEff","numID","meanStre","sumStre","idSimGlob",paste0(variables,"_Homoph"))
colnames(fullShort)[1:length(begNames)] <- begNames
fullShort <- as.data.frame(fullShort)
```


# Individual Differences [STILL NEED TO DO]
...
fullShort <- fullLong %>%
  group_by(subID) %>%
  summarise(vad_pos = mean(vad_pos, na.rm=T),
            vad_neg = mean(vad_neg, na.rm=T),
            vad_comp = mean(vad_comp, na.rm=T)) %>% full_join(fullShort, by = "subID")
```
