...
```{r, echo=FALSE}
m<-lmer( scale(negative) ~  scale(outdegree) + scale(indegree) + numID + scale(length) + ( scale(outdegree) + scale(indegree) | subID), data=fullData)
tbl_regression(m) %>% as_gt()
```
...
```{r}
predictions <- predict(m, testFull, allow.new.levels=T)
Metrics::rae(scale(testFull$Cert[!is.na(testFull$Cert) & !is.na(predictions)]), predictions[!is.na(testFull$Cert) & !is.na(predictions)])
```
...
## Exploratory: Both indegree and outdegree predict fundamental. Does outdegree depend on indegree?

Yes, outdegree is more predictive at lower levels of indegree
...
```{r}
predictions <- predict(m, testFull, allow.new.levels=T)
Metrics::rae(scale(testFull$Chan[!is.na(testFull$Chan) & !is.na(predictions)]), predictions[!is.na(testFull$Chan) & !is.na(predictions)])
```
